...
st.header('Predict Car Price using ML')

cars_data = pd.read_csv(r'C:/Users/mithu/Documents/MLproj/Cardetails.csv')
...
seller_type = st.selectbox('Seller', cars_data['seller_type'].unique())
transmission = st.selectbox('Transmission', cars_data['transmission'].unique())
owner = st.selectbox('Owner', cars_data['owner'].unique())
mileage = st.slider('Mileage', 5, 40)
engine = st.slider('Engine CC', 700, 7000)
max_power = st.slider('Max Power', 0, 700)
...
if st.button("Predict"):
    ...
    st.markdown('Car Price is Rs. ' + str(car_price[0]))